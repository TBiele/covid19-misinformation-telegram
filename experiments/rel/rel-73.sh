...
MISINFO_PRE_MODEL_NAME=smanjil/German-MedBERT
...
echo "Starting experiment ${MISINFO_PRE_MODEL_NAME}_${RUN_ID}"
...
if [[ ${MISINFO_TRAIN} = true ]]; then
    ...
    python rel/train.py \
      ...
      --model_name MISINFO-${DATASET}-${MISINFO_PRE_MODEL_NAME}_${RUN_ID} \
...
if [[ ${MISINFO_PREDICT} = true ]]; then
    ...
    python rel/predict.py \
      ...
      --pre_model_name ${MISINFO_PRE_MODEL_NAME} \
      --model_name MISINFO-${DATASET}-${MISINFO_PRE_MODEL_NAME}_${RUN_ID} \
      --output_path ${ARTIFACTS_PATH}/${MISINFO_PRE_MODEL_NAME}_${RUN_ID}_DEV \
      --max_seq_len ${MISINFO_MAX_SEQ_LEN} \
      --eval_batch_size ${MISINFO_EVAL_BATCH_SIZE} \
    ...
    python rel/predict.py \
      ...
      --pre_model_name ${MISINFO_PRE_MODEL_NAME} \
      --model_name MISINFO-${DATASET}-${MISINFO_PRE_MODEL_NAME}_${RUN_ID} \
      --output_path ${ARTIFACTS_PATH}/${MISINFO_PRE_MODEL_NAME}_${RUN_ID}_TEST \
      --max_seq_len ${MISINFO_MAX_SEQ_LEN} \
      --eval_batch_size ${MISINFO_EVAL_BATCH_SIZE} \
      --gpus ${MISINFO_EVAL_GPUS}
